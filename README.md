...
| `MOSS_AUDIO_TOKENIZER_DTYPE` | `auto` | Codec precision. `auto` = backbone dtype on CUDA / `float32` on CPU. Force `bfloat16` or `float16` to roughly halve codec VRAM; drop to `float32` only if you hear artifacts. |
...
- **Model weights are large** — ~16 GB for 8B bf16, ~3.5 GB for VoiceGenerator. First-start downloads take a while; **always** mount `/root/.cache/huggingface`. Set `MOSS_QUANTIZATION=int8` (≈½ VRAM) or `int4` (≈¼) to fit the 8B checkpoints on smaller GPUs; CUDA-only, bitsandbytes is pre-installed in the CUDA image. Quantization applies to the transformer backbone only — the MOSS-Audio-Tokenizer is a Conv-based codec that bitsandbytes does not cover; if VRAM is still tight (12 GB class cards with 8B int8), pair `MOSS_QUANTIZATION=int4` with `MOSS_AUDIO_TOKENIZER_DTYPE=bfloat16` and/or `MOSS_AUDIO_TOKENIZER_DEVICE=cpu`.
