...
| `MOSS_QUANTIZATION` | `none` | Optional bitsandbytes weight-only quantization (CUDA only): `int8` (≈½ VRAM), `int4` (NF4 + double-quant, ≈¼ VRAM). Silently downgrades to `none` on CPU. Applies only to the transformer backbone; the audio tokenizer is a Conv-based codec that bitsandbytes cannot replace — use the two knobs below to shrink it. |
| `MOSS_AUDIO_TOKENIZER_DEVICE` | `auto` | `auto` follows the backbone. Set to `cpu` to offload the MOSS-Audio-Tokenizer — saves ~1-2 GB VRAM at the cost of a short CPU encode/decode per request (fine for non-streaming). |
| `MOSS_AUDIO_TOKENIZER_DTYPE` | `auto` | Codec precision. `auto` keeps the checkpoint's native dtype. Forcing `bfloat16` / `float16` can crash with `dtype mismatch` because the upstream codec forward mixes params with non-param fp32 tensors (STFT, feature extraction). Leave it on `auto` unless you have verified the combo works — prefer `MOSS_AUDIO_TOKENIZER_DEVICE=cpu` to save VRAM. |
...
- **Model weights are large** — ~16 GB for 8B bf16, ~3.5 GB for VoiceGenerator. First-start downloads take a while; **always** mount `/root/.cache/huggingface`. Set `MOSS_QUANTIZATION=int8` (≈½ VRAM) or `int4` (≈¼) to fit the 8B checkpoints on smaller GPUs; CUDA-only, bitsandbytes is pre-installed in the CUDA image. Quantization applies to the transformer backbone only — the MOSS-Audio-Tokenizer is a Conv-based codec that bitsandbytes does not cover. If VRAM is still tight (e.g. 8B + int8 on a 12 GB card), add `MOSS_AUDIO_TOKENIZER_DEVICE=cpu` to offload the codec (saves ~1-2 GB); **do not** force `MOSS_AUDIO_TOKENIZER_DTYPE=bfloat16` — the upstream codec has dtype-sensitive forward paths and will raise `dtype mismatch`.
